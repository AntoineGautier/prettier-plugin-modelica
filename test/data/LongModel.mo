...
model AirToWater
  ...
  Buildings.Templates.Components.Actuators.Valve valChiWatMinByp(
    ...
    final typ=if have_valChiWatMinByp
  ...
  Buildings.Templates.Components.Sensors.DifferentialPressure dpChiWatLoc(
    redeclare final package Medium=MediumChiWat,
    text_rotation=90)
    if have_chiWat and not ctl.have_senDpChiWatRemWir
    "Local CHW ∆p sensor"
    annotation(Placement(transformation(extent={{-10,-10},{10,10}},
      rotation=-90,
      origin={560.5,40})));
  ...
  Buildings.Templates.Components.Actuators.Valve valHeaWatMinByp(
    ...
    final typ=if have_valHeaWatMinByp
      then Buildings.Templates.Components.Types.Valve.TwoWayModulating
      else Buildings.Templates.Components.Types.Valve.None,
    final chaTwo=Buildings.Templates.Components.Types.ValveCharacteristicTwoWay.Linear,
    final allowFlowReversal=allowFlowReversal,
  ...
  Buildings.Templates.Components.Sensors.DifferentialPressure dpHeaWatLoc(
    redeclare final package Medium=MediumHeaWat,
    text_rotation=90)
    if have_heaWat and not ctl.have_senDpHeaWatRemWir
    "Local HW ∆p sensor"
    annotation(Placement(transformation(extent={{-10,-10},{10,10}},
      rotation=-90,
      origin={560.5,-320})));
...
equation
  ...
  connect(pumChiWatPri.ports_b, outPumChiWatPri.ports_a)
    annotation(Line(points={{0,80},{0,80}},
      color={0,127,255}));
  ...
  connect(pumPri.ports_bChiHeaWat, valIso.ports_aChiHeaWatHp)
    annotation(Line(points={{-350,-50},{-350,-50}},
      color={0,127,255}));
  connect(pumPri.ports_bHeaWat, valIso.ports_aHeaWatHp)
    annotation(Line(points={{-366,-50},{-366,-50}},
      color={0,127,255}));
  connect(pumPri.ports_bChiWat, valIso.ports_aChiWatHp)
    annotation(Line(points={{-334,-50},{-334,-50}},
      color={0,127,255}));
  ...
  connect(pumChiWatSec.ports_b, outPumChiWatSec.ports_a)
    annotation(Line(points={{270,80},{270,80}},
      color={0,127,255}));
  ...
  connect(port_bChiWat, dpChiWatLoc.port_a)
    annotation(Line(points={{600,80},{560.5,80},{560.5,50}},
      color={0,127,255},
      visible=false));
  connect(dpChiWatLoc.port_b, port_aChiWat)
    annotation(Line(points={{560.5,30},{560.5,0},{600,0}},
      color={0,127,255},
      visible=false));
  connect(port_bHeaWat, dpHeaWatLoc.port_a)
    annotation(Line(points={{600,-280},{560.5,-280},{560.5,-310}},
      color={0,127,255},
      visible=false));
  connect(dpHeaWatLoc.port_b, port_aHeaWat)
    annotation(Line(points={{560.5,-330},{560.5,-360},{600,-360}},
      color={0,127,255},
      visible=false));
  connect(pumHeaWatPri.ports_b, outPumHeaWatPri.ports_a)
    annotation(Line(points={{0,-280},{0,-280}},
      color={0,127,255}));
  ...
  connect(VHeaWatPri_flow.port_b, THeaWatPriSup.port_a)
    annotation(Line(points={{40,-280},{50,-280}},
      color={0,0,0},
      thickness=0.5));
  connect(pumHeaWatSec.ports_b, outPumHeaWatSec.ports_a)
    annotation(Line(points={{270,-280},{270,-280}},
      color={0,127,255}));
  connect(inlPumHeaWatSec.ports_b, pumHeaWatSec.ports_a)
    annotation(Line(points={{250,-280},{250,-280}},
      color={0,127,255}));
  ...
  connect(TChiWatPriRet.port_b, valIso.port_aChiWat)
    annotation(Line(points={{50,0},{0,0},{0,40},{-60,40}},
      color={0,0,0},
      pattern=LinePattern.Dash,
      thickness=0.5,
      visible=have_chiWat));
  connect(junHeaWatHrcEnt.port_2, junHeaWatHrcLvg.port_1)
    annotation(Line(points={{490,-360},{390,-360}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash));
  ...
  connect(junHeaWatHrcEnt.port_3, hrc.port_a1)
    annotation(Line(points={{500,-350},{500,-180},{380,-180},{380,-160}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash,
      visible=have_hrc));
  ...
  connect(junChiWatHrcEnt.port_3, hrc.port_a2)
    annotation(Line(points={{500,-10},{500,-88}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash,
      visible=have_hrc));
  connect(junChiWatHrcEnt.port_2, junChiWatHrcLvg.port_1)
    annotation(Line(points={{490,0},{390,0}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash,
      visible=have_chiWat));
  ...
  connect(junHeaWatBypSup.port_3, valHeaWatMinByp.port_a)
    annotation(Line(points={{180,-290},{180,-310}},
      color={0,0,0},
      thickness=0.5));
  ...
  connect(junHeaWatBypRet.port_2, tanHeaWatRet.port_a)
    annotation(Line(points={{170,-360},{140,-360}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash));
  connect(tanHeaWatRet.port_b, THeaWatPriRet.port_a)
    annotation(Line(points={{120,-360},{70,-360}},
      color={0,0,0},
      thickness=0.5,
      pattern=LinePattern.Dash));
...
<ul>
  ...
  Polygon(points={{560,36},{560,0},{561,0},{561,36}},
    lineColor={0,0,0},
    visible=have_chiWat and not ctl.have_senDpChiWatRemWir),
  Polygon(points={{560,-324},{560,-360},{561,-360},{561,-324}},
    lineColor={0,0,0},
    visible=have_heaWat and not ctl.have_senDpHeaWatRemWir),
  Polygon(points={{560,-280},{560,-316},{561,-316},{561,-280}},
    lineColor={0,0,0},
    visible=have_heaWat and not ctl.have_senDpHeaWatRemWir)}));
end AirToWater;
