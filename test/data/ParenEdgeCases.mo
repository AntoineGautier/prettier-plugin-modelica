...
model ParenEdgeCases
  ...
  parameter Real test1 = if outer_cond
    then (if inner_var ==
  ...
  parameter Real test2 = if have_feature
    then (if param1 == Pkg.Type.Value then base * factor + offset
      else base / divisor - adjustment)
    else fallback;
  // Edge case 3: Nested parenthesized expressions with ifs
  parameter Real test3 = if cond1
    then ((if cond2 == Value.A then x else y) + (if cond3 == Value.B then a
      else b))
    else default;
  // Edge case 4: If in else branch with parenthesized nested if
  parameter Real test4 = if first_condition then first_result
  ...
  parameter Real test5 = if enabled
  ...
  parameter Real test6 = if use_complex
    then computeValue(
      param1=base_value,
      param2=(if variant == Library.Types.Variant.SpecialCase then special_param
        else normal_param))
    else simple_value;
  // Edge case 7: Logical operators with parenthesized nested if
  parameter Boolean test7 = if condition_a
  ...
  parameter Real test8 = if has_array
  ...
  parameter Real test9 = if level1
    then (if level2
  ...
  parameter Real test10 = if outer
    then (if flag and variable == LongPackage.Module.Type.EnumValue
      or other_flag then result_a else result_b)
    else result_c;
end ParenEdgeCases;
